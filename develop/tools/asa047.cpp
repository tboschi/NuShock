...
//
	for ( ; ; )
	{
		...
		y[n] = fn ( start , userdata, sign);
		//std::cout << fn(start, userdata,sign) << std::endl;
		//std::cout << "last " << y[n] << std::endl;
		*icount = *icount + 1;
...
//
			if ( ystar < ylo )
			{
				...
				y2star = fn ( p2star , userdata, sign);
				*icount = *icount + 1;
//
//	Check extension.
//
