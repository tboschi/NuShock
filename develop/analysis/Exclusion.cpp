...
Exclusion::Exclusion(Engine* TE, Engine::Current HornType, 
	...
	vFlag(vF),
	Thr(Threshold),
	fModifier(mod)		//modifier for Function method
{
}
...
double Exclusion::Function(double lu2)
{
	...
	{
		case Engine::FHC:
			ret = TheEngine->MakeSampler(TheBox, Engine::FHC) - Thr;
			break;
		case Engine::RHC:
			ret = TheEngine->MakeSampler(TheBox, Engine::RHC) - Thr;
			break;
		case Engine::Both:
			ret = TheEngine->MakeSampler(TheBox) - Thr;
			break;
		default:
			ret = 0.0;
			break;
