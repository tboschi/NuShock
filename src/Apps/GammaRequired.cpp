...
int main(int argc, char** argv)
{
 	...
 	//for (Mass = 0.0249; Mass < 0.0251; Mass += 0.01)	//increase mass linearly
	{
		...
		bS.resize(int(10/0.1), false);	//number of Uus probing
		for (double logUp = 0.0; logUp+1e-6 < 3.0; logUp += 0.1)
		{
			double Up = pow(10.0, logUp);
			EvGen->SetEnhancement(Up);

			vSignal.clear();
			vSignal.resize(int(10/0.01));	//number of Uus probing
			...
			{
				unsigned int i = 0;
				for (double logUu2 = -10.0; logUu2+1e-6 < 0.0; logUu2 += 0.01, ++i)	//increase Uu logarithmically
				{
			...
			unsigned int j = 0;
			for (double logUu2 = -10.0; logUu2+1e-6 < 0.0; logUu2 += 0.01, ++j)	//increase Uu logarithmically
			{
				Uu = pow(10.0, 0.5*logUu2);
				Out << Mass << "\t" << Uu*Uu << "\t" << Up << "\t" << vSignal.at(j) << std::endl;
			}
		}
	}
