...
CardDealer::CardDealer(bool verbosity) :
	kVerbosity(verbosity)
{
}

CardDealer::CardDealer(std::string cardFile, bool verbosity) :
	kVerbosity(verbosity)
{
	ReadCard(cardFile);
}
...
bool CardDealer::ReadCard(const std::string cardFile)
{
	...
	while (std::getline(inFile, line))
	{
		...
		bool isString = false;	//true if string, flase if number or array
		bool isNumber = false;

		if (ssl.peek() == '{')	//accepted character for array
			ssl.ignore();
		else if (ssl.peek() == '\"')	//accepted characters for string string start
		{
			isString = true;
			ssl.ignore();
		}

		if (ssl.peek() == '.')	//line starting with . can be a path ( ./ or ../) or a double ( .0321 )
		{
			ssl.get();
			if (ssl.peek() != '/' && ssl.peek() != '.')
				isNumber = true;	//must be a double

			ssl.seekg(-1, std::ios::cur);	//go back one
		}

		if (isString)	//there are quotation marks
			std::getline(ssl, token, '\"');		//until " char
		else if (!isNumber && !std::isdigit(ssl.peek()) &&	//there are no quotation marks
				ssl.peek() != '-' && ssl.peek() != '+')	// and no digits
			std::getline(ssl, token);		//until end of line
...
bool CardDealer::GetAll(std::map<std::string, std::vector<double> >& mv)
{
	mv.clear();
	mv = mKeyDouble;
	...
	{
		if(kVerbosity)
			std::cerr << "CardDealer::Get \" map empty" << std::endl;
		return false;
	}
}

bool CardDealer::GetAll(std::map<std::string, double>& mv)
{
	mv.clear();

	for (imd = mKeyDouble.begin(); imd != mKeyDouble.end(); ++imd)
		mv[imd->first] = imd->second.front();
	...
	{
		if(kVerbosity)
			std::cerr << "CardDealer::GetAll map empty" << std::endl;
		return false;
	}
}

bool CardDealer::GetAll(std::map<std::string, std::string>& ms)
{
	ms.clear();
	ms = mKeyString;

	if(kVerbosity)
		std::cout << "CardDealer::GetAll map empty" << std::endl;
}
