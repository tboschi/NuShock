...
Amplitude::Amplitude()	: //Decay rates calculator
	M_Neutrino(0.0),
	M_Photon(0.0),
	M_Electron(Const::MElectron),
	M_Muon(Const::MMuon),
	M_Pion(Const::MPion),
	M_Pion0(Const::MPion0),
	M_Kaon(Const::MKaon),
	M_Kaon0(Const::MKaon0),
	M_Eta(Const::MEta),
        M_Rho(Const::MRho),
        M_Rho0(Const::MRho0),
        M_Omega(Const::MOmega),
        M_Kaonx(Const::MKaonx),
        M_Etai(Const::MEtai),
        M_Phi(Const::MPhi),
        M_Tau(Const::MTau),
        M_Charm(Const::MCharm),
        M_CharmS(Const::MDs)
{
	M_Sterile_prev = -1.0;
...
{
	return M2 * Mass() / (2048 * Const::pi5);	//integration over dsigma dtau
}
...
{
	return M2 * SqrtKallen(1, x, y) / (64 * Const::pi2 * Mass());
}
...
//double Amplitude::M2_MesonThree(int Hel, double s, double t, double x, double y, double z, double L_, double L0)
{
	...
	return Const::GF2 * Mass(4) * ( (F*F) * A + (G*G) * B - (F*G) * C );
}
